...
export default function EventPreview({ onViewAllEvents }) {
  ...
  return (
    <section
      ref={sectionRef}
      className={`bg-gradient-to-t from-[#5f8861] via-[#476649] to-[#263628] py-16 px-6 transition-all duration-700 ease-in-out transform ${
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="space-y-6">
          {events.map((event) => (
            <div
              key={event.id}
              className="bg-white/10 backdrop-blur-sm rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 border border-white/20 cursor-pointer"
              onClick={() => onViewAllEvents("event-detail", event.id)}
            >
              <div className="flex flex-col md:flex-row">
                {/* Date Column */}
                <div className="bg-white/20 text-white p-4 flex flex-row md:flex-col items-center justify-center md:w-24">
                ...
                <div className="p-4 md:p-6 flex-1">
                  <h3 className="text-lg md:text-xl font-semibold text-white mb-2 hover:text-white/80 transition-colors">
                    {event.title}
                  </h3>
        ...
        <div className="text-center mt-10">
          <button
            ...
            className="bg-white/20 hover:bg-white/30 text-white font-semibold px-8 py-3 rounded-full transition-colors duration-300 backdrop-blur-sm border border-white/20 hover:border-white/40"
          >
            View All Events
          </button>
        </div>
